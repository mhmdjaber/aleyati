  ...
  rows.forEach(row => {
    ...
    if (!machines[machineId]) {
      machines[machineId] = {
        ...
        img: fixImagePath(row.main_image),
    ...
    if (machine.hasSubtypes) {
      ...
      if (subtypeId && !machine.subtypes[subtypeId]) {
        machine.subtypes[subtypeId] = {
          ...
          img: fixImagePath(row.subtype_image),
          counters: row.counters,
          dates: {}
        };
      }
...
function populateDatesPage(machineObj, machineNumber, subtypeObj = null) {
  const counterValue = subtypeObj ? subtypeObj.counters : machineObj.counters;
  const bigNumberText =
    counterValue && counterValue !== '0'
      ? `💥 ${counterValue} units`
      : machineNumber.padStart(2, '0');
    ...
    block.addEventListener('click', () => {
      ...
      date.hours.forEach(hour => {
        ...
        hourBlock.addEventListener('click', () => {
          ...
          document.getElementById('modalImage').src = fixImagePath(imgSrc);
...
function loadMachine(id) {
  ...
  if (currentMachine.hasSubtypes) {
    ...
    document.getElementById('subtypesBigNumber').textContent =
      machineCounter && machineCounter !== '0'
        ? `💥 ${machineCounter} units`
        : id.padStart(2, '0');
    ...
    currentMachine.subtypes.forEach(sub => {
      ...
      card.innerHTML = `
        <img src="${fixImagePath(sub.img)}" alt="${sub.name}">
        <h3>${sub.name}</h3>
      `;
